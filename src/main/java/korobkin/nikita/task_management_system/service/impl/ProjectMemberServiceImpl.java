...
import korobkin.nikita.task_management_system.mapper.ProjectMemberMapper;
...
public class ProjectMemberServiceImpl implements ProjectMemberService {
    ...
    public void updateProjectMember(@Valid UpdateProjectMemberRoleRequest request, User currentUser) {
        if (request.getUserId().equals(currentUser.getId())) {
            throw new IllegalArgumentException("You cannot update yourself");
        }
    ...
    public List<ProjectMemberResponse> getProjectMembers(Long projectId, User currentUser) {
        if (!projectMemberRepository.existsByProjectIdAndUserId(projectId, currentUser.getId())) {
            throw new IllegalArgumentException("You are not member of this project");
        }

        List<ProjectMember> projectMembers = projectMemberRepository.findByProjectId(projectId);

        return projectMemberMapper.toDtoList(projectMembers);
    }
